...
class ReciveData:
    ...
    def ctreate_ThreadPool(self):
        ...
        if not recived is None:
            ...
            username, pwd, equip_code, t, number, theme, description,resume = rec.split(SEPARATOR)
            
            u_name = '\'%s\'' % username
            u_pwd = '\'%s\'' % pwd
            sql = 'select id from wx_user where name=%s and password=%s' %(u_name,u_pwd)
            cursor.execute(sql)
            ...
            if len(data) < 5:
                ...
                ###############此处直接调用信息处理类来处理信息############################
                if resume == 'False':
                    d=Data_deal(user_id,number,equip_id,description,theme)
                    d.update_info()   
    ...
    def re(self, username,user_id, t):
        ...
        while 1:  # 循环接收文件
            ...
            if count == 0:  # count说明传过来的是一个文件的文件名文件大小等信息
                ...
                print(f"所有文件总大小：{afile_size}<-------->每次接收文件大小：{file_size}")
...
# 数据库信息处理类
class Data_deal():
    def __init__(self,user_id,number, equip_id,description,theme):
    ...
    def update_info(self):
        upload_data = upload_img(user_id= self.user_id, number=self.number,equip_id=self.equip_id,
